...
router.post('/command', requireRetellSecret, async (req, res) => {
    ...
    try {
        ...
        sessionManager.update({
            ...
            url: status.url,
            title: status.title,
            lastResult: `Started browser command: ${command}`,
        });
...
router.get('/context', requireRetellSecret, (req, res) => {
    return res.json({ success: true, context: sessionManager.get().pageContext, session: sessionManager.get() });
});

router.get('/status', requireRetellSecret, (req, res) => {
    const status = agentService.getStatus();
    const session = sessionManager.get();
    const state = status.running ? 'running' : (session.state || (status.lastError ? 'failed' : 'idle'));
    return res.json({
        success: true,
        running: status.running,
        state,
        message: status.lastError || session.lastResult || (status.running ? 'Browser is working.' : 'No browser command is running.'),
        status,
        progress: session,
    });
});
